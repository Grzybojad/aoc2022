...
int solve_day4_2(std::string input_filename) {
    ...
    for (size_t i = 0; i < lines.size(); i++) {
        ...
        pairs[0] = line.substr(0, line.find(pair_delimiter));
        pairs[1] = line.substr(line.find(pair_delimiter)+1, line.length());

        int ranges[2][2];

        for (size_t j = 0; j < 2; j++) {
            int delimeter_pos = pairs[j].find(range_delimeter);
            ranges[j][0] = stoi(pairs[j].substr(0, delimeter_pos));
            ranges[j][1] = stoi(pairs[j].substr(delimeter_pos+1, pairs[j].length()-delimeter_pos));
        }
